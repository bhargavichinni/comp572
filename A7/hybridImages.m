...
set2_far = im2double(imread('https://raw.githubusercontent.com/bhargavichinni/comp572/main/A7/Set6/ferris.JPG'));  % "far" picture
set2_near = im2double(imread('https://raw.githubusercontent.com/bhargavichinni/comp572/main/A7/Set6/oysters.JPG')); % "near" picture
...
cutoff_high = 5;   % provide a value for the high-pass filter cutoff frequency (sigma of gaussian)
...
function im12 = hybridImage(im1, im2, cutoff_low, cutoff_high)
    ...
    % low image
    [h, w, c] = size(im1);
    gauss_low = fspecial("gaussian", [h, w], cutoff_low);
    gauss_low = gauss_low / max(gauss_low(:));  % need to scale so max of gaussian is 1
    if c > 1, gauss_low = repmat(gauss_low,[1 1 c]); end
    I_fft_low = im1 .* gauss_low;
    I_low = real(ifft2(ifftshift(I_fft_low)));  % apply filter 

    % high image
    gauss_high = fspecial("gaussian", [h, w], cutoff_high);
    gauss_high = gauss_high / max(gauss_high(:));  % scale the high-pass filter
    if c > 1, gauss_high = repmat(gauss_high,[1 1 c]); end
    I_fft_high = im2 .* (1 - gauss_high);  % apply high-pass filter
    I_high = real(ifft2(ifftshift(I_fft_high)));  % apply filter to the second image

    % combine images
    im12 = I_low + I_high;
    im12 = im12 - min(im12(:));
    m = max(im12(:)); if m>0, im12 = im12/m; end
end
